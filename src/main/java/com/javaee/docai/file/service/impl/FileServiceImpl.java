package com.javaee.docai.file.service.impl;

import com.javaee.docai.doc.service.DocumentFileService;
import com.javaee.docai.file.config.FileStorageConfig;
import com.javaee.docai.file.config.MinioConfig;
import com.javaee.docai.file.service.FileMetadataService;
import com.javaee.docai.file.service.FileService;
import com.javaee.docai.file.util.FileUtils;
import com.javaee.docai.file.util.Md5Utils;
import com.javaee.docai.file.util.PathUtils;
...
public class FileServiceImpl implements FileService {
    ...
    public String upload(MultipartFile file) {
        try {
            ...
            try {
                com.javaee.docai.file.entity.FileMetadata fileMetadata = new com.javaee.docai.file.entity.FileMetadata();
    ...
    public String uploadDocument(MultipartFile file) {
        // 校验文档类型：仅允许PDF/Word/PPT/CSV
        String contentType = file.getContentType();
        if (!isDocumentType(contentType)) {
            throw new RuntimeException("文档管理仅支持PDF、Word、PPT和CSV类型文件");
        }
        // 复用上传逻辑
    ...
    private String uploadInternal(MultipartFile file) {
        try {
            ...
            try {
                ...
                try {
                    createBy = com.javaee.docai.security.UserContext.getCurrentUsername();
    ...
    public String mergeChunk(String fileId, String fileName) {
        try {
            ...
            try {
                ...
                fileMetadata.setFileType(com.javaee.docai.file.util.FileUtils.getContentType(fileName));
    ...
    public byte[] download(String fileId) {
        try {
            // 尝试从数据库获取文件元数据
            com.javaee.docai.file.entity.FileMetadata fileMetadata = null;
    ...
    public String copy(String fileId, String targetPath) {
        try {
            ...
            try {
                if (fileMetadata != null) {
                    com.javaee.docai.file.entity.FileMetadata newFileMetadata = new com.javaee.docai.file.entity.FileMetadata();
    ...
    /**
     * 判断是否为文档类型（PDF/Word/PPT/CSV）
     */
    private boolean isDocumentType(String contentType) {
        if (contentType == null) return false;
        String type = contentType.toLowerCase();
        return type.contains("pdf") || type.contains("word") || type.contains("msword")
                || type.contains("csv")
